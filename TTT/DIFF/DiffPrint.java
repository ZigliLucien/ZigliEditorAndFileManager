...
//      setupOutput();
  ...
  public static class NormalPrint extends Base {
	...
	return;
      ...
      tmpout.append(change_letter(inserts, deletes));
	  ...
	  for (int i = first1; i <= last1; i++)
	    {
	      ...
	      if (! inserting)
		tmpout.append(i - first1 + first0 + "a\n");
	      inserting = true;
	      ...
	      if (".".equals(file1[i]))
		{
		  ...
		  tmpout.append(i - first1 + first0 + 1 + "s/^\\.\\././\n");
		  inserting = false;
		}
	      else
  ...
  public static class ContextPrint extends Base {
	...
	tmpout.append(mark + ' ' + inf.getPath()+"\n");
    }
...
public DiffPrint(String one, String two) throws IOException{

	this(new String[]{one,two});


}

  public DiffPrint(String[] argv) throws IOException {

	tmpout = new StringBuilder(8193);
...
public static void main(String[] qubs){
try{

	new DiffPrint(qubs);

	System.out.println(DIFF.DiffPrint.results);
