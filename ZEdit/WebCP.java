...
public class WebCP extends TPEditorX implements ActionListener {
    ...
    static InputStream GetIt;
    static OutputStream FileIt;
	 URLConnection urla;
    ...
    static JTextField textin = new JTextField("Welcome to the Web!");

    CalHTMLPreferences cpref;
    CalHTMLPane viewer;
     CHOLocal choc;
...
//	HELLO
	...
	cpref = new CalHTMLPreferences();

	cpref.setHomeURL(homeURL);
	cpref.setUnderlineLinks(false);
	cpref.setFormRenderingStyle(CalCons.USE_CALPA_FLUSH);

            choc = new ZEdit.CHOLocal(textin,this);
	viewer = new CalHTMLPane(cpref, choc,"ZEFMView");
...
try{
		if (currentURL != null) displayURL(new URL(textin.getText()));
}catch(Exception texte){}
                    }
...
*/
               ...
               if (!e.isControlDown() && !e.isShiftDown()) {
                     	if (currentURL != null) textin.setText(currentURL.toString());
                    }
                            if (!e.isShiftDown() && e.isControlDown()) {
                                 moveToBack();
                            }
       ...
       jiji.add(Box.createHorizontalGlue());
        ...
        menuItem = new JMenuItem("Load file for Edit");
...
*/
                    ...
                    try {
                        ...
                        try {
                            if (ctype.equals("text/html") ||
                                ...
                                viewer.showHTMLDocument(currentURL);
                            } else {
                                ...
                                if (webinfo.containsKey(ctype)) {
                                    ...
                                    String filename = userdir + "/TmpZefm/" + name;
                                    ...
                                    if (ctype.startsWith("image")) {
                                        FileIt = new FileOutputStream(userdir + "/tmp.html");
                                        ...
                                        FileIt.flush();
                                        FileIt.close();
	                            viewer.showHTMLDocument("file:" + userdir + "/tmp.html");
                                    } else {
                                           runIt(xrun, filename);
                                           running = true;
                                    }
            ...
            case (1):
                ...
                try {
                    ...
                    final Frame winji = new Frame("Downloading " +
                            url.toString());
                    TextArea outview = new TextArea("DOWNLOADING\n\n", 20, 70);
                ...
                if (nn == 1) {
                    ...
                    viewer.showHTMLDocument(new URL("file:" + userdir + "/" + filename));
                }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        try {
            if (called.equals("Back Button") || called.equals("Back")) {
                caught = true;
	    ...
	    viewer.goBack();
            ...
            if (called.equals("Download")) {
                caught = true;
                mm = Download(currentURL);
            }
	...
	if (called.equals("Load file for Edit")) {

                        String hostt = currentURL.getHost() + ":" + currentURL.getPort();

                     boolean local = (currentURL.getHost().equals("localhost") ||
                            InetAddress.getByName(currentURL.getHost()).getHostAddress().equals(localaddr));

                        if (!local) {
		                            doEdit(hostt, currentURL);
                            return;
                        }

                        if (local) {
		String addrs = currentURL.toString();
		int cut = addrs.indexOf("http://");
		int cut2 = addrs.substring(cut+7).indexOf("/");
		String localfile = addrs.substring(cut+7).substring(cut2);

                    TPEditorX newBuffer = new TPEditorX(localfile,localfile);
...
//////////////////////// DoPage /////////////////////////////////////
     ...
     void doPage(URLConnection _urla) {
        try {
            ...
            FileIt.flush();
            FileIt.close();
            viewer.showHTMLDocument(new URL("file:" + filename));
        } catch (Exception e) {
            return;
        }
    }
...
try{         
         URLConnection urlee = _url.openConnection();

        InputStream getIt = urlee.getInputStream();
